...
if (isset($_SESSION['id'])&&isset($_SESSION['username'])&&isset($_SESSION['password'])&&isset($_SESSION['first_name'])&&isset($_SESSION['last_name'])&&isset($_POST['id'])) {
	...
	$q="SELECT p.id AS `Patient ID`, p.first_name AS `First Name`, p.last_name AS `Last Name`, d.trade_name AS `Drug Trade Name`, d.generic_name AS `Drug Generic Name`, pd.start_date AS `Start Date`, _doctor.last_name AS `Doctor` FROM _patient AS p INNER JOIN _is_on_drug AS pd ON p.id=pd.patient_id INNER JOIN _drug AS d ON pd.drug_id=d.id INNER JOIN _doctor ON p.doctor_id=_doctor.id WHERE d.id='".$mysqli->real_escape_string($_POST['id'])."' AND _doctor.last_name='".$_SESSION['last_name']."'";
